...
/* 보내기: 폰·공기계는 공유창(카톡·텔레그램 선택), PC는 복사 */
async function copyText(txt){if(navigator.share){try{await navigator.share({text:txt});return}catch(e){if(e&&e.name==='AbortError')return}}
  try{await navigator.clipboard.writeText(txt);toast('복사했습니다. 카톡이나 텔레그램에 붙여넣으세요.')}catch(e){prompt('복사해서 쓰세요',txt)}}
...
async function home(){const d=today();const v=$('#view');const mgr=isMgr();
  v.innerHTML=`<div class="card notice"><h2>📣 잊지 말고! <span class="tip">전체 알림</span><span class="sp"></span>${mgr?'<button class="btn sm pri" id="ntAdd">+ 알림</button>':''}</h2><div id="ntList" class="tip">…</div></div>
    ...
    ${mgr?`<div class="card"><h2>📦 발주 확인 <button class="btn sm" style="margin-left:auto" data-go="stock">열기</button></h2><div id="ordPending" class="tip">…</div></div><div class="card"><h2>체크리스트 진행 <button class="btn sm" style="margin-left:auto" data-go="check">열기</button></h2><div id="ckProg"></div></div>`:''}`;
  ...
  S.unsub.push(DB.watch('stock',[['month','==',d.slice(0,7)]],rows=>{state.st=rows.find(r=>r.date===d)||null;renderFlow();const box=$('#ordPending');if(!box)return;
    const latest=rows.sort((a,b)=>b.date.localeCompare(a.date))[0];const items=[];if(latest)(latest.orders||'').split('\n').map(l=>l.trim()).filter(Boolean).forEach(l=>{const c=(latest.orderChecks||{})[l]||{};if(!c.ordered)items.push({l,ok:c.ok,from:c.from})});
    box.innerHTML=items.length?`<p class="tip" style="margin:0 0 6px">주문 안 된 발주 <b>${items.length}</b>건 (${latest.date.slice(5)} 기준)</p>`+items.slice(0,8).map(i=>`<div class="ev"><span>${esc(i.l)}${i.from?` <span class="tip">${i.from.slice(5)}부터</span>`:''}${i.ok?' <span class="tag blue">확인됨</span>':''}</span></div>`).join('')+(items.length>8?`<p class="tip">외 ${items.length-8}건</p>`:''):'<span class="tip">주문 대기 중인 발주가 없습니다.</span>'}));
...
async function check(){const v=$('#view');let tab=shiftNow();let date=today();let editMode=false;
  ...
  async function renderCk(){const id=`${date}_${tab}`;const box=$('#ckBody');
    S.unsub.push(DB.watch('checklists',[['date','==',date],['shift','==',tab]],async rows=>{let doc=rows[0];const fresh=!doc;if(fresh)doc={date,shift:tab,items:parseTemplate(await getTemplate(tab))};
      ...
      box.innerHTML=`<div class="row" style="margin-bottom:6px"><b>${dn}/${items.length}</b><span class="tip">${editMode?'✏️ 편집 중: ✎ 글 고치기 · × 빼기 · + 넣기. 바로 저장되고 다음 날부터도 적용됩니다.':p===100?'🎉 끝! 텔레그램 보고만 남았어요.':'하면서 체크만 하면 됩니다.'}</span><span class="sp"></span>${editMode?'<button class="btn sm" data-addh>+ 소제목</button>':'<button class="btn sm" id="ckCopy">📤 보내기</button>'}</div><div class="bar"><i style="width:${p}%"></i></div>
...
async function stockCheck(){const box=$('#stBody');let cur=null,curStock={},curChecks={},custom=(await DB.get('stockConfig','main'))?.customItems||[];let month=today().slice(0,7);
  ...
  box.innerHTML=`<div class="two"><div><div class="card"><h2>체크 목록 <span class="sp"></span><button class="btn sm pri" id="newOrd">+ 오늘</button></h2><div class="row" style="margin-bottom:8px"><input type="month" id="ordMonth" value="${month}"></div><div class="list" id="ordList"></div></div>
    ...
    <div><div class="card" id="ordForm"><h2>발주·재고 체크 <span id="ordTitle" class="tag"></span><span class="tip" id="ordStatus"></span><span class="sp"></span><button class="btn sm pri" id="copyOrd">📤 보내기</button>${isMgr()?'<button class="btn sm danger noprint" id="delOrd">삭제</button>':''}</h2>
      <p class="tip">어제 숫자가 들어 있어요. <b>바뀐 것만</b> 고치면 자동 저장됩니다. 주황색은 발주 넣을 때.</p>
      <h3>🛒 발주 넣을 것</h3><div id="autoNeed" class="row" style="margin-bottom:6px"></div><textarea id="o_orders" style="min-height:70px" placeholder="한 줄에 하나씩&#10;난황 10개&#10;에타용 휘핑크림 12통"></textarea>
      <div id="orderChecks" style="margin-top:8px"></div>
  ...
  async function open(d){cur=d;let o=await DB.get('stock',d);if(!o){const all=await DB.query('stock');const prev=all.filter(x=>x.date<d).sort((a,b)=>a.date.localeCompare(b.date)).pop();o={date:d,month:d.slice(0,7),by:S.me.name,stock:prev?{...prev.stock}:{},creamPlan:prev?.creamPlan||''};
      // 주문 안 된 발주 줄은 다음 날로 이월 (확인 상태·처음 적힌 날짜 유지)
      if(prev){const pc=prev.orderChecks||{};const carry=(prev.orders||'').split('\n').map(l=>l.trim()).filter(l=>l&&!(pc[l]||{}).ordered);if(carry.length){o.orders=carry.join('\n');o.orderChecks={};carry.forEach(l=>{o.orderChecks[l]={...(pc[l]||{}),from:(pc[l]||{}).from||prev.date}})}}}
    OF.forEach(k=>{$('#o_'+k).value=o[k]??''});$('#o_date').value=d;if(!o.by)$('#o_by').value=S.me.name;curStock={...(o.stock||{})};curChecks={...(o.orderChecks||{})};$('#ordTitle').textContent=`${d} (${dow(d)})`;$('#ordStatus').textContent='';renderStock();renderChecks();renderList()}
  const read=()=>{const o={stock:{...curStock},orderChecks:{...curChecks}};OF.forEach(k=>{o[k]=$('#o_'+k).value});o.month=o.date.slice(0,7);return o};
  /* 발주 확인: 사장·점장·매니저가 각 줄을 "확인" → "주문 완료" 로 체크. 직원은 상태만 봄 */
  function renderChecks(){const box=$('#orderChecks');if(!box)return;const lines=($('#o_orders').value||'').split('\n').map(l=>l.trim()).filter(Boolean);
    if(!lines.length){box.innerHTML='';return}const mgr=isMgr();
    box.innerHTML=`<div class="tip" style="margin-bottom:4px">발주 확인 ${mgr?'(확인 → 주문 완료 순서로 체크)':''} · 주문 안 된 줄은 다음 날로 넘어갑니다</div><ul class="ck oc">${lines.map(l=>{const c=curChecks[l]||{};
      return `<li class="${c.ordered?'dn':''}"><span class="t">${esc(l)}<small>${c.from?`${c.from.slice(5)}부터 · `:''}${c.ok?`확인 ${esc(c.by)} ${esc(c.at)}`:''}${c.ordered?` · 주문 완료 ${esc(c.oBy)} ${esc(c.oAt)}`:''}</small></span>
        ${mgr?`<label class="ocb"><input type="checkbox" data-ok="${esc(l)}" ${c.ok?'checked':''}> 확인</label><label class="ocb"><input type="checkbox" data-ord="${esc(l)}" ${c.ordered?'checked':''}> 주문함</label>`:(c.ordered?'<span class="tag green">주문 완료</span>':c.ok?'<span class="tag blue">확인됨</span>':'<span class="tag">대기</span>')}</li>`}).join('')}</ul>`;
    $$('[data-ok]',box).forEach(cb=>cb.onchange=()=>{const l=cb.dataset.ok;const c=curChecks[l]||(curChecks[l]={});c.ok=cb.checked;c.by=cb.checked?S.me.name:'';c.at=cb.checked?nowHM():'';if(!cb.checked){c.ordered=false}$('#o_orders').dispatchEvent(new Event('change',{bubbles:true}));renderChecks()});
    $$('[data-ord]',box).forEach(cb=>cb.onchange=()=>{const l=cb.dataset.ord;const c=curChecks[l]||(curChecks[l]={});c.ordered=cb.checked;c.oBy=cb.checked?S.me.name:'';c.oAt=cb.checked?nowHM():'';if(cb.checked&&!c.ok){c.ok=true;c.by=S.me.name;c.at=nowHM()}$('#o_orders').dispatchEvent(new Event('change',{bubbles:true}));renderChecks()})}
  $('#o_orders').addEventListener('input',renderChecks);
  ...
  $('#copyOrd').onclick=async()=>{await save();const o=read();const n=needs();const g=x=>allItems().filter(y=>y[0]===x&&curStock[y[1]]!==''&&curStock[y[1]]!=null).map(y=>y[1]+' '+curStock[y[1]]+y[2]).join(', ')||'-';
    const ordTxt=(o.orders||'').split('\n').map(l=>l.trim()).filter(Boolean).map(l=>{const c=curChecks[l]||{};return (c.ordered?'✅ ':c.ok?'☑ ':'⬜ ')+l}).join('\n');
...
/* ==================== 일지 + 생산량 + 입고 ==================== */
const MEMO_TAGS=[{e:'⏱',n:'시각'},{e:'😠',n:'컴플레인',c:'var(--red)'},{e:'⚠️',n:'이슈'},{e:'🔧',n:'고장',c:'var(--red)'},{e:'📦',n:'입고 안 됨',c:'var(--red)'},{e:'🗑',n:'폐기',c:'var(--red)'},{e:'🎁',n:'서비스',c:'var(--blue)'},{e:'📞',n:'예약·주문',c:'var(--blue)'},{e:'✅',n:'보고용'}];
const LF=['date','weather','staffOpen','staffMid','staffClose','staffOff','bossWork','flow','best','full','complaint','visit','service','waste','egg','whole','star','pieceCnt','bingsu','piece','produced','tomorrow','inSheet','inCream','inFruit','closeStock','orderNote','notice','cleaning','report','memo'];
async function log(){const v=$('#view');let cur=null;let month=today().slice(0,7);
  v.innerHTML=`<div class="two"><div><div class="card"><h2>일지 목록 <span class="sp"></span><button class="btn sm pri" id="newLog">+ 오늘</button></h2><div class="row" style="margin-bottom:8px"><input type="month" id="logMonth" value="${month}"></div><div class="list" id="logList"></div></div>
    ...
    <div><div class="card" id="logForm"><h2>일지 <span id="logTitle" class="tag"></span><span class="tip" id="logStatus"></span><span class="sp"></span><button class="btn sm pri" id="copyLog">📤 보내기</button>${isMgr()?'<button class="btn sm danger noprint" id="delLog">삭제</button>':''}</h2>
      <p class="tip">하루 중엔 메모만, 마감 때 숫자. 자동 저장돼요.</p>
      <h3>🗒 오늘 매장 <span class="tip">날씨·손님 흐름 한 줄 + 메모</span></h3>
      ...
      <div class="row noprint memo-tags" style="margin:10px 0 6px">${MEMO_TAGS.map(t=>`<button class="btn sm" data-ins="${t.e} " style="color:${t.c||'var(--ink)'}">${t.e} ${t.n}</button>`).join('')}</div>
      <textarea class="tall" id="f_memo" placeholder="😠 14:10 자리 없다고 화내심, 죄송하다고 안내&#10;🔧 업소용 냉장고 소리 큼&#10;✅ 파키스탄 망고 다음주 수·금 입고 → 월요일 2박스 더 받기로 함&#10;📦 파키스탄 망고 수·금 입고로 밀림&#10;🗑 감자파이 1개 뭉개져 폐기&#10;🎁 감자파이 1개 서비스&#10;📞 에타 30구 당일 예약 문의 → 2시까지 가능 안내"></textarea>
      <div style="margin-top:6px"><div class="memo-view" id="memoView" hidden></div></div>
<h3>🍰 오늘 숫자</h3><div class="grid"><div><label>에그타르트 구운 갯수</label><input type="number" id="f_egg" inputmode="numeric"></div><div><label>홀케이크</label><input type="number" id="f_whole" inputmode="numeric"></div><div><label>조각케이크</label><input type="number" id="f_pieceCnt" inputmode="numeric"></div><div><label>빙수</label><input type="number" id="f_bingsu" inputmode="numeric"></div><div><label>별의조각</label><input type="number" id="f_star" inputmode="numeric"></div>
  ...
  async function renderList(){month=$('#logMonth').value;const rows=(await DB.query('logs',[['month','==',month]])).sort((a,b)=>b.date.localeCompare(a.date));
    $('#logList').innerHTML=rows.length?rows.map(l=>`<button class="${l.date===cur?'on':''}" data-d="${l.date}">${l.date.slice(5)} (${dow(l.date)})<small>${esc(l.updatedBy||'')} ${esc((l.memo||'').split('\n')[0]).slice(0,36)}</small></button>`).join(''):'<p class="tip">이 달 일지가 없습니다.</p>';
    $$('#logList [data-d]').forEach(b=>b.onclick=()=>open(b.dataset.d));
    if(!isMgr())return;const sum=k=>rows.reduce((a,l)=>a+(Number(l[k])||0),0);$('#logKpi').innerHTML=[['영업일',rows.length,'일'],['에그타르트',sum('egg'),'개'],['홀케이크',sum('whole'),'개'],['조각케이크',sum('pieceCnt'),'개'],['빙수',sum('bingsu'),'개'],['별의조각',sum('star'),'개']].map(([t,n,u])=>`<div><span>${t}</span><b>${n}<small style="font-size:11px;font-weight:400"> ${u}</small></b></div>`).join('');
