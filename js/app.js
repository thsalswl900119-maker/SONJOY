...
function renderNav(){$('#navTop').innerHTML=VIEWS.map(([k,n])=>`<button data-v="${k}" class="${S.view===k?'on':''}">${n}</button>`).join('');
  $('#navBottom').innerHTML=VIEWS.filter(x=>x[0]!=='settings').map(([k,n,i,sn])=>`<button data-v="${k}" class="${S.view===k?'on':''}"><b>${i}</b>${sn||n}</button>`).join('');
...
/* ==================== 홈 ==================== */
async function home(){const d=today();const v=$('#view');
  v.innerHTML=`<div class="card"><h2>${d} (${dow(d)}) <span class="clock" id="clock" style="margin-left:auto"></span></h2>
    <div id="attBox"></div></div>
    <div class="two half"><div class="card"><h2>오늘 근무 <button class="btn sm" style="margin-left:auto" data-go="shifts">근무표</button></h2><div id="todayShifts" class="tip">불러오는 중…</div></div>
    <div class="card"><h2>체크리스트 진행 <button class="btn sm" style="margin-left:auto" data-go="check">열기</button></h2><div id="ckProg"></div></div></div>
    <div class="two half"><div class="card"><h2>다가오는 스케줄 (2주) <button class="btn sm" style="margin-left:auto" data-go="sched">전체</button></h2><div id="upcoming"></div></div>
    <div class="card"><h2>오늘 기록 <span class="sp"></span></h2><div id="todayDocs"></div></div></div>`;
  ...
  renderAttBox();
  S.unsub.push(DB.watch('shifts',[['date','==',d]],rows=>{const box=$('#todayShifts');if(!box)return;const w=rows.filter(r=>!r.off).sort((a,b)=>(a.start||'').localeCompare(b.start||''));const off=rows.filter(r=>r.off);
  ...
  S.unsub.push(DB.watch('checklists',[['date','==',d]],rows=>{const box=$('#ckProg');if(!box)return;box.innerHTML=['open','mid','close'].map(k=>{const doc=rows.find(r=>r.shift===k);const items=(doc?.items||[]).filter(i=>i.type!=='h');const dn=items.filter(i=>i.done).length;const p=items.length?Math.round(dn/items.length*100):0;
  ...
  const [logDoc,stDoc]=await Promise.all([DB.get('logs',d),DB.get('stock',d)]);
  $('#todayDocs').innerHTML=`<div class="ev"><span>📝 일지</span><span class="sp"></span>${logDoc?`<span class="tag green">작성됨 · ${esc(logDoc.updatedBy||'')}</span>`:'<span class="tag red">미작성</span>'}<button class="btn sm" data-go="log">열기</button></div>
    <div class="ev"><span>📦 발주·재고 체크</span><span class="sp"></span>${stDoc?`<span class="tag green">작성됨 · ${esc(stDoc.updatedBy||'')}</span>`:'<span class="tag red">미작성</span>'}<button class="btn sm" data-go="stock">열기</button></div>`;
  $$('#todayDocs [data-go]').forEach(b=>b.onclick=()=>go(b.dataset.go));
  ...
  $('#upcoming').innerHTML=evs.length?evs.map(e=>`<div class="ev"><span class="dt">${e.date.slice(5)} (${dow(e.date)})</span><span>${SCHED_CATS[e.cat]?.slice(0,2)||''} ${esc(e.title)}${e.endDate?` <span class="tip">~${e.endDate.slice(5)}</span>`:''}</span></div>`).join(''):'<p class="tip">2주 내 스케줄이 없습니다.</p>';
}
/* 출근/퇴근 박스 (홈·출퇴근 공용) */
async function renderAttBox(){const box=$('#attBox');if(!box)return;const d=today();const id=`${S.me.id}_${d}`;const a=await DB.get('attendance',id);const shift=(await DB.query('shifts',[['date','==',d]])).find(s=>s.uid===S.me.id);
  box.innerHTML=`<div class="row" style="margin-bottom:8px"><b>${esc(S.me.name)}</b>${shift?`<span class="tag ${shift.off?'':'blue'}">${shift.off?'오늘 휴무':'근무 '+shift.start+'~'+shift.end}</span>`:'<span class="tag">근무표 없음</span>'}
    ${a?.in?`<span class="tag green">출근 ${a.in}</span>`:''}${a?.out?`<span class="tag green">퇴근 ${a.out}</span>`:''}${a?.in&&a?.out?`<span class="tag">${hours(a.in,a.out)}시간</span>`:''}</div>
    ...
    <p class="tip" style="margin:8px 0 0">출근은 매장 도착 기준 10분 전. 시간이 잘못 찍혔으면 출퇴근 메뉴에서 메모를 남기고 관리자에게 말해주세요.</p>`;
  ...
  $('#btnOut').onclick=async()=>{if(!confirm(`${nowHM()} 퇴근 처리할까요? 마감 일지·텔레그램 보고는 하셨나요?`))return;await DB.update('attendance',id,{out:nowHM()});toast('퇴근 완료. 수고하셨습니다!');renderAttBox()};
}
...
async function check(){const v=$('#view');let tab='open';let date=today();
  v.innerHTML=`<div class="card"><h2>체크리스트 <input type="date" id="ckDate" value="${date}" style="width:150px"><span class="sp"></span>${isMgr()?'<button class="btn sm" id="ckTpl">템플릿 편집</button>':''}<button class="btn sm" id="ckAdd">+ 오늘만 추가</button></h2>
    <div class="subtabs" id="ckTabs">${['open','mid','close'].map(k=>`<button data-t="${k}">${DEFAULT_CHECKLISTS[k].title} <span class="tip">${DEFAULT_CHECKLISTS[k].time}</span></button>`).join('')}<button data-t="clean">🧹 월간 청소</button></div>
  ...
  async function renderCk(){const id=`${date}_${tab}`;const box=$('#ckBody');
    S.unsub.push(DB.watch('checklists',[['date','==',date],['shift','==',tab]],async rows=>{let doc=rows[0];
      if(!doc){doc={date,shift:tab,items:parseTemplate(await getTemplate(tab))};box.innerHTML=`<p class="tip">${date} ${DEFAULT_CHECKLISTS[tab].title} 체크리스트를 아직 시작하지 않았습니다.</p><button class="btn pri" id="ckStart">${DEFAULT_CHECKLISTS[tab].title} 체크 시작</button>`;$('#ckStart').onclick=()=>DB.set('checklists',id,doc);return}
      ...
      box.innerHTML=`<div class="row" style="margin-bottom:6px"><b>${dn}/${items.length} 완료</b><span class="tip">${p===100?'🎉 모두 완료! 텔레그램 보고 잊지 마세요.':''}</span><span class="sp"></span><button class="btn sm" id="ckCopy">📋 보고용 복사</button></div><div class="bar"><i style="width:${p}%"></i></div>
        <ul class="ck">${doc.items.map((it,i)=>it.type==='h'?`<li class="h">${esc(it.t)}</li>`:`<li class="${it.done?'dn':''}"><input type="checkbox" data-i="${i}" ${it.done?'checked':''}><span class="t">${esc(it.t)}${it.done?`<small>✓ ${esc(it.by)} ${esc(it.at)}</small>`:''}</span>${it.adhoc?`<button class="btn sm danger" data-rm="${i}">×</button>`:''}</li>`).join('')}</ul>`;
      $$('[data-i]',box).forEach(cb=>cb.onchange=async()=>{const i=Number(cb.dataset.i);const it=doc.items[i];it.done=cb.checked;it.by=cb.checked?S.me.name:'';it.at=cb.checked?nowHM():'';await DB.update('checklists',id,{items:doc.items})});
      $$('[data-rm]',box).forEach(b=>b.onclick=async()=>{doc.items.splice(Number(b.dataset.rm),1);await DB.update('checklists',id,{items:doc.items})});
...
async function stockCheck(){const box=$('#stBody');let cur=null,curStock={},custom=(await DB.get('stockConfig','main'))?.customItems||[];let month=today().slice(0,7);
  ...
  box.innerHTML=`<div class="two"><div><div class="card"><h2>체크 목록 <span class="sp"></span><button class="btn sm pri" id="newOrd">+ 오늘 체크</button></h2><div class="row" style="margin-bottom:8px"><input type="month" id="ordMonth" value="${month}"></div><div class="list" id="ordList"></div><p class="tip" style="margin-top:8px">새 체크를 만들면 직전 체크의 재고 수량이 복사됩니다. 바뀐 것만 고치세요.</p></div>
    <div class="card noprint"><h2>이번 주 사용량 <span class="tag" id="weekLabel"></span></h2><p class="tip">월~토 사용량. 토요일 마감 전에 이 숫자로 주 단위 발주를 넣고 가세요.</p><label>과일 (알 / 병)</label><div class="week" id="weekFruit"></div><label style="margin-top:8px">생크림 (통)</label><div class="week" id="weekCream"></div></div></div>
    <div><div class="card"><h2>발주·재고 체크 <span id="ordTitle" class="tag"></span><span class="sp"></span><button class="btn sm" id="copyOrd">📋 텔레그램용 복사</button><button class="btn sm danger noprint" id="delOrd">삭제</button><button class="btn sm pri noprint" id="saveOrd">저장</button></h2>
      <div class="grid"><div><label>날짜</label><input type="date" id="o_date"></div><div><label>작성자</label><input type="text" id="o_by"></div></div>
      <h3>🛒 발주 넣어야 할 최종 목록</h3><div class="tipbox">바로 시켜야 하거나 주문해야 하는 것들을 정리. 발주 목록은 캡처해서 텔레그램방에 꼭 올리기 (누락 이슈 있음). 아래 재고에서 발주점 이하로 떨어진 품목은 자동 표시됩니다.</div>
      <div id="autoNeed" class="row" style="margin-bottom:6px"></div><textarea id="o_orders" placeholder="난황 (아이엠에그, 10개)&#10;에타용 휘핑크림 (베이킹맘, 12~17통)"></textarea>
      <h3>📣 총괄 보고할 특이사항</h3><textarea id="o_report" style="min-height:70px" placeholder="생산·재고·발주 이슈. 예: 업소용 냉장고 고장 → 컴프레서 교체 / 키위 3일 전에 미리 말하기"></textarea>
      <h3>🍰 생산량 파악 / 조각케이크</h3><div class="tipbox">케이크 생산량을 늘려서 많이 팔 수 있는 쪽으로 항상 생각하고, 폐기는 최대한 줄이기. 안 나가는 품목은 최소 재고 2개 유지 후 조절.</div>
      <div class="grid"><div class="w2"><label>오늘 생산 (예: 망2 무피2 초체1 복1 멜1)</label><input type="text" id="o_prod"></div><div class="w2"><label>생산 메모</label><input type="text" id="o_prodMemo" placeholder="재고 많아 1개씩 줄임 / 냉장고 티4 있음"></div></div>
      <h3>🍞 시트 <span class="tip">가게 폰으로 문자 주문 · 상태 이상하면 바로 거래처 연락 + 사진</span></h3><div class="stock" id="st_sheet"></div><div style="margin-top:6px"><input type="text" id="o_sheetMemo" placeholder="시트 메모: 목요일 도착으로 기본 6개 주문함"></div>
      <h3>🥭 1) 과일 (메인과일)</h3><div class="tipbox">망고 1박스 최대 58,000원 넘으면 안 됨. 후숙 과일은 1~1.5박스 여유 (당일 케이크+빙수). 조림 복숭아 반박스 남으면 미리 주문. 주마다 과일별 단가 체크 후 일지에 기록. 파손 시 바로 A/S. 5월 망고값 상승 → 6월 애플망고 전환, 6월 중순 복숭아, 11월 초 딸기 확보, 11월 중순 크리스마스 케이크 촬영.</div>
      <div class="grid"><div><label>오늘 과일 사용 (알)</label><input type="number" id="o_fruitUseA" step="0.5"></div><div><label>오늘 과일 사용 (병)</label><input type="number" id="o_fruitUseB" step="0.5"></div><div class="w2"><label>과일 주문 갯수 · 입고 요일</label><input type="text" id="o_fruitOrder" placeholder="월요일 망고 2박스, 대추토마토 대과 4kg"></div></div>
      <div class="stock" id="st_fruit" style="margin-top:8px"></div><div style="margin-top:6px"><input type="text" id="o_fruitMemo" placeholder="과일 메모: 망고 1박스 58,000원 입고"></div>
      <h3>🥛 2) 생크림 <span class="tip">녹산대리점(메인) · 3사 택배(서브) · 2일 전 문자</span></h3><div class="grid"><div><label>오늘 생크림 사용 (통)</label><input type="number" id="o_creamUse"></div><div class="w3"><label>입고 예정</label><input type="text" id="o_creamPlan" placeholder="월·수·금 20통씩"></div></div><div class="stock" id="st_cream" style="margin-top:8px"></div>
      <h3>☕ 3~5) 우유 · 원두 · 휘핑크림</h3><div class="stock" id="st_bar"></div>
      <h3>🥚 7) 매일 쓰는 부재료</h3><div class="stock" id="st_sub"></div>
      <h3>📦 8) 포장 재료 <span class="tip">케이크 상자·조각 상자 수량 제대로 파악</span></h3><div class="stock" id="st_pack"></div>
      <h3>🍧 9) 기타 부재료 · 빙수 (4~10월)</h3><div class="tipbox">공통: 연유. 우유+말차팥: 빙수팥/인절미가루/인절미다이스/모나카(일본 10일 전)/말차가루. 토마토: 토마토청(반통 남으면 담그기, 숙성 3~4일)/통후추/홀토마토/레몬즙. 망고: 다크블라썸/냉동망고/코코넛밀크/패션후르츠퓨레.</div>
      <div class="stock" id="st_etc"></div><div style="margin-top:6px"><input type="text" id="o_etcMemo" placeholder="기타 메모"></div>
      ${isMgr()?'<div class="row noprint" style="margin-top:12px"><button class="btn sm" id="addItem">+ 재고 항목 추가</button><span class="tip">추가한 항목은 모든 체크에 표시됩니다 (매니저 이상).</span></div>':''}
    </div></div></div>`;
  ...
  function renderStock(){Object.keys(STOCK_GROUPS).forEach(g=>$('#st_'+g).innerHTML='');
    allItems().forEach(([g,name,unit,re,memo,isCustom])=>{const val=curStock[name]??'';const need=re!=null&&val!==''&&Number(val)<=re;const el=document.createElement('div');el.className='it'+(need?' need':'');
      el.innerHTML=`<span class="nm">${esc(name)}${re!=null||memo?`<small>${re!=null?'발주점 ≤'+re+unit:''}${memo?' · '+esc(memo):''}</small>`:''}</span><input type="number" step="0.5" value="${esc(val)}"><span class="u">${unit}</span>${isCustom&&isMgr()?'<button class="btn sm danger" title="항목 삭제">×</button>':''}`;
  ...
  function renderNeed(){const n=needs();$('#autoNeed').innerHTML=n.length?`<span class="tag red">발주점 이하 ${n.length}개</span>`+n.map(x=>`<button class="btn sm" data-add="${esc(x)}">+ ${esc(x)}</button>`).join(''):'<span class="tag green">발주점 이하 품목 없음</span>';
  ...
  async function open(d){cur=d;const o=(await DB.get('stock',d))||{date:d,stock:{}};OF.forEach(k=>{$('#o_'+k).value=o[k]??''});$('#o_date').value=d;if(!o.by)$('#o_by').value=S.me.name;curStock={...(o.stock||{})};$('#ordTitle').textContent=`${d} (${dow(d)})`;renderStock();renderList()}
  const read=()=>{const o={stock:{...curStock}};OF.forEach(k=>{o[k]=$('#o_'+k).value});o.month=o.date.slice(0,7);return o};
  $('#newOrd').onclick=async()=>{const d=today();if(!(await DB.get('stock',d))){const all=await DB.query('stock');const prev=all.filter(x=>x.date<d).sort((a,b)=>a.date.localeCompare(b.date)).pop();await DB.set('stock',d,{date:d,month:d.slice(0,7),by:S.me.name,stock:prev?{...prev.stock}:{},creamPlan:prev?.creamPlan||''})}$('#ordMonth').value=d.slice(0,7);open(d)};
  $('#saveOrd').onclick=async()=>{const o=read();if(!o.date)return alert('날짜를 입력하세요');if(cur&&cur!==o.date)await DB.del('stock',cur);await DB.set('stock',o.date,o);cur=o.date;$('#ordMonth').value=o.month;toast('저장했습니다');renderList()};
  $('#delOrd').onclick=async()=>{if(cur&&confirm(cur+' 체크를 삭제할까요?')){await DB.del('stock',cur);cur=null;curStock={};OF.forEach(k=>$('#o_'+k).value='');$('#ordTitle').textContent='';renderStock();renderList()}};
  ...
  $('#copyOrd').onclick=()=>{const o=read();const n=needs();const grp=g=>allItems().filter(x=>x[0]===g&&curStock[x[1]]!==''&&curStock[x[1]]!=null).map(x=>x[1]+' '+curStock[x[1]]+x[2]).join(', ')||'-';
    copyText(`[발주·재고 체크] ${o.date} (${dow(o.date)}) ${o.by||''}\n■ 발주 최종 목록\n${o.orders||'-'}${n.length?'\n(발주점 이하: '+n.join(', ')+')':''}\n■ 총괄 보고 특이사항\n${o.report||'-'}\n■ 조각케이크 생산: ${o.prod||'-'} ${o.prodMemo||''}\n■ 시트: ${grp('sheet')} ${o.sheetMemo||''}\n■ 과일 사용 ${o.fruitUseA||0}알 / ${o.fruitUseB||0}병 · 재고: ${grp('fruit')}\n  주문: ${o.fruitOrder||'-'} ${o.fruitMemo||''}\n■ 생크림 사용 ${o.creamUse||0}통 · 재고 ${grp('cream')} · ${o.creamPlan||''}\n■ 우유·원두·휘핑: ${grp('bar')}\n■ 부재료: ${grp('sub')}\n■ 포장: ${grp('pack')}\n■ 빙수·기타: ${grp('etc')} ${o.etcMemo||''}`)};
  renderStock();await renderList();const all=await DB.query('stock',[['month','==',month]]);const last=all.sort((a,b)=>a.date.localeCompare(b.date)).pop();if(last)open(last.date);
}
...
async function log(){const v=$('#view');let cur=null;let month=today().slice(0,7);
  v.innerHTML=`<div class="two"><div><div class="card"><h2>일지 목록 <span class="sp"></span><button class="btn sm pri" id="newLog">+ 오늘 일지</button></h2><div class="row" style="margin-bottom:8px"><input type="month" id="logMonth" value="${month}"></div><div class="list" id="logList"></div></div>
    <div class="card noprint"><h2>이번 달 합계</h2><div class="kpi" id="logKpi"></div><div class="row"><button class="btn sm" id="csvLog">CSV</button><button class="btn sm" id="printLog">인쇄</button></div></div></div>
    <div><div class="card"><h2>운영일지 <span id="logTitle" class="tag"></span><span class="sp"></span><button class="btn sm" id="copyLog">📋 텔레그램용 복사</button><button class="btn sm danger noprint" id="delLog">삭제</button><button class="btn sm pri noprint" id="saveLog">저장</button></h2>
      <div class="tipbox">${esc(LOG_TIPS)}</div>
      <div class="grid"><div><label>날짜</label><input type="date" id="f_date"></div><div><label>날씨</label><input type="text" id="f_weather" placeholder="맑음 / 흐림 / 비"></div><div><label>오픈</label><input type="text" id="f_staffOpen" list="staffNames"></div><div><label>미들</label><input type="text" id="f_staffMid" placeholder="알바 12-4시"></div><div><label>마감</label><input type="text" id="f_staffClose" list="staffNames"></div><div><label>휴무</label><input type="text" id="f_staffOff"></div><div class="w2"><label>사장님 업무</label><input type="text" id="f_bossWork" placeholder="외부 강의 준비 / 촬영"></div></div>
      <datalist id="staffNames">${S.roster.map(u=>`<option value="${esc(u.name)}">`).join('')}</datalist>
      <h3>👥 매장 흐름</h3><div class="grid"><div class="w2"><label>손님 흐름 (몇 명이 자주, 몰린 시간)</label><input type="text" id="f_flow" placeholder="오전 픽업·포장 위주, 12시부터 몰림, 2시 만석"></div><div class="w2"><label>잘 나간 메뉴</label><input type="text" id="f_best" placeholder="무화과 케이크, 빙수 골고루"></div><div><label>만석·웨이팅</label><input type="text" id="f_full" placeholder="2시 만석, 대기 2팀"></div><div><label>컴플레인</label><input type="text" id="f_complaint"></div><div class="w2"><label>관공서·사장님 찾는 방문</label><input type="text" id="f_visit"></div></div>
      <h3>💰 매출 · 배달 · 서비스 · 폐기</h3><div class="grid"><div><label>중간 매출 (15시 이전)</label><input type="number" id="f_midSales"></div><div><label>총매출 (원, 네이버 포함)</label><input type="number" id="f_sales"></div><div><label>배민 (건)</label><input type="number" id="f_baemin"></div><div><label>쿠팡 (건)</label><input type="number" id="f_coupang"></div><div><label>네이버 주문 (건)</label><input type="number" id="f_naver"></div><div class="w3"><label>서비스 나간 것</label><input type="text" id="f_service" placeholder="감자파이 1개 서비스"></div><div class="w2"><label>폐기</label><input type="text" id="f_waste" placeholder="감자파이 1개 뭉개짐 → 직원 나눠먹음"></div></div>
      <h3>🍰 생산량</h3><div class="grid"><div><label>에그타르트 구운 갯수</label><input type="number" id="f_egg"></div><div><label>홀케이크</label><input type="number" id="f_whole"></div><div><label>별의조각 (들어온 갯수)</label><input type="number" id="f_star"></div><div><label>조각케이크 갯수</label><input type="number" id="f_pieceCnt"></div><div><label>빙수 갯수</label><input type="number" id="f_bingsu"></div><div class="w3"><label>조각케이크 내역 (아침 생산 / 전일 재고)</label><input type="text" id="f_piece" placeholder="망17,멜9,복요8,무피8,초체4,티4 / 전일자 티4,망1,멜1"></div><div class="w2"><label>오늘 생산한 케이크</label><input type="text" id="f_produced"></div><div class="w2"><label>내일 주문 들어온 케이크</label><input type="text" id="f_tomorrow"></div></div>
      <h3>📦 입고 · 남은 재고 · 발주</h3><div class="grid"><div><label>케이크 시트 입고</label><input type="number" id="f_inSheet"></div><div><label>생크림 입고</label><input type="number" id="f_inCream"></div><div><label>과일 입고</label><input type="number" id="f_inFruit"></div><div class="w3"><label>마감 남은 재고 (에타 / 케이크 / 망고 / 생크림)</label><input type="text" id="f_closeStock" placeholder="망4,감자2 남음 / 망고 19알, 생크림 11통, 복숭아 2.5병 사용"></div><div class="w3"><label>재료 발주 관련 (과일·생크림)</label><input type="text" id="f_orderNote" placeholder="파키스탄 망고 수·금 입고, 월요일 망고 2박스 더"></div></div>
      <h3>📣 공지 · 청소 · 보고</h3><div class="grid"><div class="w2"><label>직원들에게 알릴 내용 / 공지</label><input type="text" id="f_notice"></div><div class="w2"><label>청소 여부</label><input type="text" id="f_cleaning" placeholder="화장실·쇼케이스 바닥 완료"></div><div class="w3"><label>사장님께 보고할 사항</label><textarea id="f_report" style="min-height:60px"></textarea></div></div>
      <h3>🗒 메모사항 (시간순)</h3><div class="row noprint" style="margin-bottom:6px"><button class="btn sm" data-ins="⏱ ">⏱ 시각</button><button class="btn sm" data-ins="🔴 " style="color:var(--red)">🔴 재고·폐기·발주</button><button class="btn sm" data-ins="🔵 " style="color:var(--blue)">🔵 서비스·특이</button><button class="btn sm" data-ins="⭐ ">⭐ 고객·주문 이슈</button><button class="btn sm" data-ins="✅ ">✅ 사장님 확인</button></div>
      <textarea class="tall" id="f_memo" placeholder="⏱ 11:00 손님 전화, 당일 에타 30구 예약 문의 → 2시까지 가능 안내&#10;⏱ 13:45 매출 70만원 (쿠팡 1.4만, 배민 2.8만)&#10;🔴 픽업 냉장고 토마토청 곰팡이 → 폐기, 사장님 보고&#10;🔵 감자파이 1개 서비스&#10;⭐ 쿠팡 짭짤이토마토 세트 재고 없어 통화 후 취소"></textarea>
      <div style="margin-top:8px"><label>미리보기</label><div class="memo-view" id="memoView"></div></div></div></div></div>`;
  const renderMemo=()=>{$('#memoView').innerHTML=$('#f_memo').value.split('\n').map(l=>`<span class="${l.startsWith('🔴')?'r':l.startsWith('🔵')?'b':(l.startsWith('⭐')||l.startsWith('✅'))?'s':''}">${esc(l)}</span>`).join('\n')};
  $('#f_memo').oninput=renderMemo;
  $$('[data-ins]',v).forEach(b=>b.onclick=()=>{const t=$('#f_memo');let ins=b.dataset.ins;if(ins.startsWith('⏱'))ins+=nowHM()+' ';const s=t.selectionStart;const before=t.value.slice(0,s);const nl=(before&&!before.endsWith('\n'))?'\n':'';t.value=before+nl+ins+t.value.slice(s);t.focus();t.selectionStart=t.selectionEnd=(before+nl+ins).length;renderMemo()});
  async function renderList(){month=$('#logMonth').value;const rows=(await DB.query('logs',[['month','==',month]])).sort((a,b)=>b.date.localeCompare(a.date));
    ...
    const sum=k=>rows.reduce((a,l)=>a+(Number(l[k])||0),0);$('#logKpi').innerHTML=[['영업일',rows.length,'일'],['총매출',fmt(sum('sales')),'원'],['에그타르트',sum('egg'),'개'],['홀케이크',sum('whole'),'개'],['조각케이크',sum('pieceCnt'),'개'],['빙수',sum('bingsu'),'개'],['배달',sum('baemin')+sum('coupang')+sum('naver'),'건']].map(([t,n,u])=>`<div><span>${t}</span><b>${n}<small style="font-size:11px;font-weight:400"> ${u}</small></b></div>`).join('');
    $('#csvLog').onclick=()=>{const q=s=>'"'+String(s??'').replace(/"/g,'""')+'"';dl('﻿'+['날짜,요일,'+LF.slice(1).join(','),...rows.slice().reverse().map(l=>[l.date,dow(l.date),...LF.slice(1).map(k=>l[k])].map(q).join(','))].join('\n'),`운영일지_${month}.csv`,'text/csv')}}
  async function open(d){cur=d;const l=(await DB.get('logs',d))||{date:d};LF.forEach(k=>{$('#f_'+k).value=l[k]??''});$('#f_date').value=d;$('#logTitle').textContent=`${d} (${dow(d)})`+(l.updatedBy?` · ${l.updatedBy}`:'');renderMemo();renderList();
  ...
  $('#newLog').onclick=async()=>{const d=today();if(!(await DB.get('logs',d)))await DB.set('logs',d,{date:d,month:d.slice(0,7)});$('#logMonth').value=d.slice(0,7);open(d)};
  $('#saveLog').onclick=async()=>{const o=read();if(!o.date)return alert('날짜를 입력하세요');if(cur&&cur!==o.date)await DB.del('logs',cur);await DB.set('logs',o.date,o);cur=o.date;$('#logMonth').value=o.month;toast('저장했습니다');renderList();$('#logTitle').textContent=`${o.date} (${dow(o.date)}) · ${S.me.name}`};
  $('#delLog').onclick=async()=>{if(cur&&confirm(cur+' 일지를 삭제할까요?')){await DB.del('logs',cur);cur=null;LF.forEach(k=>$('#f_'+k).value='');$('#logTitle').textContent='';renderMemo();renderList()}};
  $('#logMonth').onchange=renderList;$('#printLog').onclick=()=>window.print();
  $('#copyLog').onclick=()=>{const o=read();const L=(t,v)=>v?`${t}: ${v}\n`:'';copyText(`[운영일지] ${o.date} (${dow(o.date)}) ${o.weather||''}\n근무: 오픈 ${o.staffOpen||'-'} / 미들 ${o.staffMid||'-'} / 마감 ${o.staffClose||'-'} / 휴무 ${o.staffOff||'-'}\n`+L('사장님',o.bossWork)+L('손님 흐름',o.flow)+L('잘 나간 메뉴',o.best)+L('만석·웨이팅',o.full)+L('컴플레인',o.complaint)+L('방문',o.visit)+
    `매출: 중간 ${fmt(o.midSales)||'-'} / 총 ${fmt(o.sales)||'-'}원 (배민 ${o.baemin||0} 쿠팡 ${o.coupang||0} 네이버 ${o.naver||0})\n`+L('서비스',o.service)+L('폐기',o.waste)+`생산: 에타 ${o.egg||0} / 홀 ${o.whole||0} / 별조각 ${o.star||0} / 조각 ${o.pieceCnt||0} / 빙수 ${o.bingsu||0}\n`+L('조각 내역',o.piece)+L('생산 케이크',o.produced)+L('내일 주문',o.tomorrow)+`입고: 시트 ${o.inSheet||0} / 생크림 ${o.inCream||0} / 과일 ${o.inFruit||0}\n`+L('남은 재고',o.closeStock)+L('발주',o.orderNote)+L('공지',o.notice)+L('청소',o.cleaning)+L('보고',o.report)+(o.memo?`\n${o.memo}`:''))};
  await renderList();const rows=await DB.query('logs',[['month','==',month]]);const last=rows.sort((a,b)=>a.date.localeCompare(b.date)).pop();if(last)open(last.date);
}
